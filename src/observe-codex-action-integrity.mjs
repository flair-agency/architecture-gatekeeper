...
export function observeWorkflowIdentity(workflow, procedure) {
  ...
  requireJobControls(workflow, 'review', "if: needs.policy.outputs.mode == 'enforced' || needs.policy.outputs.mode == 'advisory'");
  requireJobControls(workflow, 'codex-action-integrity', "if: needs.policy.outputs.mode == 'enforced' || needs.policy.outputs.mode == 'advisory'");
