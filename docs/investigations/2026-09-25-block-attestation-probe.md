...
`.github/workflows/issue83-attestation-probe.yml` is manually dispatched and writes one fixed-shape **synthetic** JSON file from GitHub runner context. It uses the official, exact-SHA-pinned `actions/attest` and `actions/upload-artifact` actions. The job has only `contents: read`, `id-token: write`, and `attestations: write`; it has no source checkout, model API key, or acceptance-path connection. The seven-day artifact exists solely so a separate read-only verifier can fetch the exact file bytes. Fields inside that file are self-claims and must not be treated as authenticated run metadata.

After the workflow is merged onto the default branch, run it manually twice, including a rerun of one run. For each attempt:
