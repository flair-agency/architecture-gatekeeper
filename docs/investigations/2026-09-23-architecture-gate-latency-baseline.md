...
The self-Gate cohort is the 11 newest `Self Architecture Gate` workflow runs visible on 2026-09-23, run numbers 45 through 55 inclusive (IDs 35828071613 through 35845043390). We count attempts separately: run #50 has both its failed attempt 1 and successful attempt 2, giving 12 measured attempts. The dates below are UTC. The selections are consecutive run numbers, not hand-picked by outcome. All 11 used `pull_request_target`.
